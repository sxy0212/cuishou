        ...
        methods: {
          ...
          node(){
            var _this = this
              this.show = true
              this.obj = []
            const url = "/api_backend.php?r=template-tree/tree-list"
            const conf = {
            url,
            data:{
              template_id:this.template_id
                        ...
                        if (value && Object.keys(value).length) {
                            ...
                            $dom.dblclick((e)=>{
                              ...
                              const conf = {
                                  ...
                                  success:(data)=>{
                                    _this.baseMessageDataEdit = data.info
                                    if( data.info.type == '0'){
                                      _this.baseMessageDataEdit.type = '0'
                                    }else if(data.info.type == '1'){
                                      _this.baseMessageDataEdit.type = '1'
                                    }
                                  }
                              }
                              axiosRequest(conf)
                            ...
                            var $name = $(
                                ...
                                !!(value.condition) && $name.append(value.condition);
                                $dom.append($name)
                                $dom.css("background-color",value.color);
                                if (value.type && value.type === 'organization') {
                                    // 详情部分
                                    var $wrap =
                                        `<div class="text" > 
                                        <p class="text_content"> ${value.answer}</p></div>
                                    `;
                            ...
                            var icon1 = '<i class="plusMinus glyphicon glyphicon-plus-sign"></i>';
                            var $icon = $(icon);
                            var $icon1 = $(icon1);
